...
### September 12 - 2026
[SECourses Video Captioner Pro - Qwen3-Omni - TimeChat Captioner GRPO - AVoCaDO - 1-Click Windows, RunPod, Massed Compute Installers](https://www.patreon.com/posts/168757767)
* What you can do with Video Captioner Pro: Caption video, images, and sound. Generate scene and motion descriptions, image captions, or audio descriptions with a model suited to the media and task. Choose the right AI for the job. Access five model families, 21 checkpoint variants, 19 workflow presets, and 52 task presets, with custom prompts and a reusable prompt library. Turn speech into subtitles. Use Whisper for transcription, English translation, and optional word timestamps; export SRT, VTT, TXT, LRC, TSV, or JSON. Ask follow-up questions about your media. Chat with Qwen3 Instruct or Thinking using video, audio, images, and text to explore details beyond the first caption. Break long videos into useful clips. Split by detected scenes, fixed duration, or trainer length; preview sampled frames and filter clips by sharpness, darkness, motion, or silence. Process folders and ZIPs in batches. Scan subfolders, filter filenames and media types, skip completed items, retry failures, and distribute compatible folder jobs across multiple GPUs. Combine visuals, speech, and sound. Keep separate video and audio caption parts, reuse existing visual captions, and merge descriptions, transcripts, and sound captions with your own templates. Review and improve captions in one editor. Browse media, filter the queue, find and replace text, apply bulk edits, regenerate captions with a before/after diff, and approve the items you want to keep. Prepare datasets for training. Export approved media/text pairs and ZIPs, inspect caption statistics and clip fitness, sub-split videos, and generate Kohya / Musubi TOML configurations. Choose a memory plan that fits your hardware. Use VRAM presets, native BF16/INT8/INT4 variants or supported Qwen GGUF variants, block swapping, and model unloading to manage GPU memory. Carry a good workflow into the next job. Save prompts and presets, recover settings from previous runs, download result files or ZIPs, and use the supplied Windows or Linux/cloud setup scripts.
...
* I have prepared literally 1-click installers and GPU specific automatic optimizer ran files. My installer will install with Torch 2.13 and CUDA 13 + pre-compiled xFormers, Flash Attention, Sage Attention and InsightFace and TorchAO. My precompiled libraries are compiled with support of all consumer GPUs + cloud GPUs for Python 3.10, 3.11, 3.12, 3.13. Currently installer installs with Python 3.13 since it is recommended so have Python 3.13 as well in your system. I have developed smart_launch.py and it will be used to launch the app on Windows, RunPod and Massed Compute and it will automatically add optimization parameters according to your GPU. The Windows bat files and RunPod and Massed Compute scripts will use it by default so don't worry just use installer files and launch files, Smart launcher supports all GPUs like RTX 1000, 2000, 3000, 4000 and 5000 series or cloud GPUs like H100, RTX PRO 6000, etc. Currently scripts are set to start with disabled Sage Attention since it produces lower quality outputs. If you enable, enable like this : --sage2-function fp16_triton. To download models quickly you can use our Unified Model Downloader (updated to support this app) : https://www.patreon.com/posts/114517862
...
### January 1 - 2026
[FaceFusion V3.5.2 (Most Advanced 1-Click DeepFake) 1-Click Installers for Windows, RunPod and Massed Compute - Use locally and cloud](https://www.patreon.com/posts/103765029)
* FaceFusion 3.5.2 Windows (local), RunPod and Massed Compute (Cloud) 1-Click installers. Install and run with easiness. The best DeepFake APP. The installation literally 1-click supports RTX 5000 series and older GPUs with CUDA and also TensorRT.
...
* This is premium installer and app for TripoSR, a state-of-the-art open-source model for fast feedforward 3D reconstruction from a single image, collaboratively developed by Tripo AI and Stability AI. I have significantly improved the official repo app. Our installer installs with pre-compiled wheels for Torch 2.8, CUDA 12.9, Flash Attention, Sage Attention, xFormers, torchmcubes, Triton and more. It is compiled to support literally all GPUs starting from RTX 1000 series to 5000 series or Cloud GPUs like A100, H200, B200, etc. Just double click and run Windows_Install_or_Update.bat. It will both install and download all models into models folder
